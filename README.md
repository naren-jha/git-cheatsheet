...
* `git rebase master` - rebase feature_branch on the tip of  current master. Note that this will create new commits instead of merging existing one with a merge commit (which is what `git merge` does)
